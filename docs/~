...
Dogker follows the [Texas hold'em](https://en.wikipedia.org/wiki/Texas_hold_%27em) variant of poker. There are no time constraints either for a player's turn or the game in general. The actual game logic is defined in `game.dto.ts`. Logic related to game management is defined in `games.service.ts`. There is also a couple of utility REST functions defined in `game.controller.ts` to obtain valid game codes and find if a player is in a specific game.
...
There are different ways of handling connection between users in a WebRTC application. A peer-to-peer connection involving 10 users would require the use of a mesh topology, which could become very resource (bandwidth, CPU) intensive on the user side: 

<img src="https://miro.medium.com/max/637/0*EgWw-gpXYh-cmXsW"
	alt="WebRTC peer-to-peer" />

As we can see, in this case, every user would need to send their audio stream to every other user (upload) and also receive and process (CPU) every other user's streams (download) at the same time.

So, with Dogker, the audiobridge plugin is capable of using an MCU (Multipoint Conferencing Unit) topology, which alleviates the resource use on the user side, with the downside of requiring more computing power on the server:

<img src="https://miro.medium.com/max/700/0*0BoXN4UTFnAc3FAO"
	alt="WebRTC MCU" />

Now, every user only sends their audio stream and also receives a single, processed and mixed audio stream.
